...
# === НАЛАШТУВАННЯ ===
TOKEN = "8500138070:AAEkY3S3T9Urn1FjesqPvxoxeSUzBHrCdiU"  # сюди вставити токен від BotFather
ADMIN_ID = 6958866740      # numeric ID
ADMIN_USERNAME = "RiKOWENS420"  # username без @
...
# === НАЛАШТУВАННЯ ===
TOKEN = "YOUR_TOKEN_HERE"  # сюди вставити токен від BotFather
...
async def show_products(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute("SELECT id, name, price FROM items WHERE category=?", (category,))
    rows = cur.fetchall()
    conn.close()
...
async def product_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute("SELECT name, price, description, image, category FROM items WHERE id=?", (pid,))
    row = cur.fetchone()
    conn.close()
